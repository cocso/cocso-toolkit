function buildFeatures(features) {
    ...
    if (features.width !== undefined)
        featurePairs.push(`width=${features.width}`);
    if (features.height !== undefined)
        featurePairs.push(`height=${features.height}`);
    if (features.top !== undefined)
        featurePairs.push(`top=${features.top}`);
    if (features.left !== undefined)
        featurePairs.push(`left=${features.left}`);
    ...
    if (features.noopener)
        featurePairs.push('noopener');
    if (features.noreferrer)
        featurePairs.push('noreferrer');
 ...
 */
export function openPopupWindow(url, options = {}) {
    if (typeof window === 'undefined' ||
        typeof window.screen === 'undefined' ||
        typeof window.open !== 'function') {
        return null;
    }
    const { target = '_blank', width = 800, height = 680, top, left, resizable = true, scrollbars = true, menubar = false, toolbar = false, location = false, status = false, directories = false, copyhistory = false, noopener = false, noreferrer = false, } = options;
    const minSize = 200;
...
//# sourceMappingURL=openPopupWindow.js.map